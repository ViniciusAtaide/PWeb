...
<div class="${tamanho }">
	...
	<c:if test="${empty result}">
		<h4 class="text-info">Não há nenhuma música com esse nome</h4>
	</c:if>
	<c:forEach items="${result}" var="musica">
		...
		<br>
		<a href="album.do?id=${musica.album.id}&action=show">${musica.album.nome }</a><br>
		<audio controls src="${musica.caminhoarq }" onplay="$.ajax('music.do?action=play&id=${musica.id }');"></audio>					
	</c:forEach>
</div>
